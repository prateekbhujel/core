...
@php
  ...
  $dbLabel = ($dbConnectionInfo['database'] ?? '') !== ''
    ? (string) $dbConnectionInfo['database']
    : 'n/a';
...
<div class="hl-docs hl-settings">
  <div class="doc-head">
    <div>
      ...
      <div class="doc-sub">
        Keep app identity, environment setup, activity, notifications, and diagnostics organized from one clean panel.
      </div>
    </div>
  ...
  <div class="h-tab-shell h-settings-shell" id="settings-main-tabs" data-ui-tabs data-default-tab="{{ $defaultSettingsTab }}">
    <div class="h-tab-nav" role="tablist" aria-label="Settings sections">
      <button type="button" class="h-tab-btn" data-tab-btn="settings-app">
      ...
      <button type="button" class="h-tab-btn" data-tab-btn="settings-activity">
        <i class="fa-solid fa-chart-line"></i>
        Activity
      </button>
      <button type="button" class="h-tab-btn" data-tab-btn="settings-notifications">
      ...
      @endif
      @if($canManageSettings)
        <button type="button" class="h-tab-btn" data-tab-btn="settings-diagnostics">
          <i class="fa-solid fa-stethoscope"></i>
          Diagnostics
        </button>
      @endif
      <button type="button" class="h-tab-btn" data-tab-btn="settings-assistant">
            ...
            <form method="POST" action="{{ route('settings.branding') }}" enctype="multipart/form-data" data-spa>
              ...
              <div class="row g-3">
                ...
                <div class="col-md-2">
                  ...
                  <input type="color" name="ui_theme_color" class="form-control form-control-color" value="{{ old('ui_theme_color', $themeColor) }}" title="Choose theme color">
                </div>
                ...
                <div class="col-md-4">
                  <label class="h-label" style="display:block;">App Icon URL (PWA / iOS)</label>
              ...
              <div class="h-note mt-3">
                Active DB from `.env`: <code>{{ $dbConnectionInfo['connection'] }}</code> /
                <code>{{ $dbConnectionInfo['database'] ?: 'n/a' }}</code>
              </div>
        ...
        <div class="h-card-soft mb-3">
          ...
          <div class="body">
            @if(empty($mediaLibrary))
              <div class="h-note">
                No branding assets uploaded yet. Upload files above and they will appear here automatically.
              </div>
            @else
              ...
              <div class="col-md-6" style="padding-top:24px;">
                <div class="h-radio-stack">
                  <label class="form-check">
                    <input class="form-check-input" type="checkbox" name="two_factor_enabled" value="1" @checked(auth()->user()->two_factor_enabled)>
                    <span class="form-check-label">Enable 2FA login</span>
                  </label>
                  <label class="form-check">
                    <input class="form-check-input" type="checkbox" name="receive_in_app_notifications" value="1" @checked(auth()->user()->receive_in_app_notifications)>
                    <span class="form-check-label">In-app notifications</span>
                  </label>
                  <label class="form-check">
                    <input class="form-check-input" type="checkbox" name="receive_telegram_notifications" value="1" @checked(auth()->user()->receive_telegram_notifications)>
                    <span class="form-check-label">Telegram notifications</span>
                  </label>
                  <label class="form-check">
                    <input class="form-check-input" type="checkbox" name="browser_notifications_enabled" value="1" @checked(auth()->user()->browser_notifications_enabled)>
              ...
              <div class="row g-3">
                ...
                <div class="col-md-4">
                  ...
                  <select
                    ...
                    data-min-input="1"
                    data-dropdown-parent="#settings-main-tabs"
                  ></select>
                </div>
              </div>

              <div class="h-note mt-3">
                Browser alerts now trigger a short sound + vibration (where supported) when new unread notifications arrive.
              </div>
    ...
    @if($canManageSettings)
      <div class="h-tab-panel" data-tab-panel="settings-diagnostics">
        @if(!$opsUiEnabled)
          <div class="h-note mb-3">
            Diagnostics shell controls are disabled. Enable <code>HAARRAY_ALLOW_SHELL_UI=true</code> if you need maintenance actions.
          </div>
        @endif

        <div class="h-card-soft mb-3">
          <div class="head">
            <div style="font-family:var(--fd);font-size:16px;font-weight:700;">Overview</div>
            <div class="h-muted" style="font-size:13px;">Environment health and runtime details.</div>
          </div>
          <div class="body">
            <div class="row g-3">
        ...
        <div class="h-card-soft mb-3">
          <div class="head">
            <div style="font-family:var(--fd);font-size:16px;font-weight:700;">Database Browser</div>
            <div class="h-muted" style="font-size:13px;">Read-only table preview from active connection.</div>
          </div>
          <div class="body">
            ...
            <form method="GET" action="{{ route('settings.index') }}" data-spa class="mb-3">
              ...
              <div class="row g-2 align-items-end">
                ...
                <div class="col-md-4">
                  <button type="submit" class="btn btn-outline-secondary w-100">Load Table</button>
                </div>
              </div>
            </form>
        ...
        <div class="h-card-soft mb-3">
          <div class="head">
            <div style="font-family:var(--fd);font-size:16px;font-weight:700;">ML Diagnostic Lab</div>
            <div class="h-muted" style="font-size:13px;">Run wrapper probe to see current classification output.</div>
          </div>
          <div class="body">
            ...
            <form method="POST" action="{{ route('settings.ml.probe') }}" data-spa>
              ...
              <div class="row g-3">
                <div class="col-md-4">
                  <label class="h-label" style="display:block;">Food Ratio</label>
                  <input type="number" step="0.01" min="0" max="1" name="food_ratio" class="form-control" value="{{ old('food_ratio', $mlDefaults['food_ratio'] ?? 0.35) }}" required>
                </div>
                <div class="col-md-4">
                  <label class="h-label" style="display:block;">Entertainment Ratio</label>
                  <input type="number" step="0.01" min="0" max="1" name="entertainment_ratio" class="form-control" value="{{ old('entertainment_ratio', $mlDefaults['entertainment_ratio'] ?? 0.20) }}" required>
                </div>
                <div class="col-md-4">
                  <label class="h-label" style="display:block;">Savings Rate</label>
                  <input type="number" step="0.01" min="0" max="1" name="savings_rate" class="form-control" value="{{ old('savings_rate', $mlDefaults['savings_rate'] ?? 0.30) }}" required>
                </div>
              </div>

              <div class="d-flex justify-content-end mt-3">
                <button type="submit" class="btn btn-outline-secondary" data-busy-text="Running...">Run ML Probe</button>
              </div>
            </form>
            ...
            @if(!empty($mlProbeResult))
              <div class="h-note mt-3">
                ...
                <pre style="margin:8px 0 0;white-space:pre-wrap;color:var(--t1);font-family:var(--fm);font-size:11px;">{{ json_encode($mlProbeResult, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
              </div>
            @endif
          </div>
        </div>
    ...
    <div class="h-tab-panel" data-tab-panel="settings-assistant">
      <div class="h-card-soft">
        <div class="head">
          <div style="font-family:var(--fd);font-size:16px;font-weight:700;">Haarray AI Desk</div>
          <div class="h-muted" style="font-size:13px;">Quick assistant for starter-kit operations. Local helper now, backend AI endpoint can be attached later.</div>
        </div>
        <div class="body">
          <div id="h-ai-chat-log" class="h-ai-log">
            <div class="h-ai-msg bot">
              <div class="h-ai-bubble">Ask about settings, RBAC, notifications, DB, or starter setup. I will guide the next step quickly.</div>
            </div>
          </div>

          <form id="h-ai-chat-form" class="h-ai-form mt-2">
            <input type="text" id="h-ai-chat-input" class="form-control" placeholder="Type your question...">
...
(function () {
  ...
  document.addEventListener('h:tabs:changed', function (event) {
    ...
    if (!event.detail.tabId) return;
    saveQuery(event.detail.tabId);
  });
  ...
  document.addEventListener('click', function (event) {
    ...
    if (window.HToast) {
      HToast.success('Selected asset applied. Save settings to persist.');
    }
  });

  const chatLog = document.getElementById('h-ai-chat-log');
  const chatForm = document.getElementById('h-ai-chat-form');
  const chatInput = document.getElementById('h-ai-chat-input');
  if (!chatLog || !chatForm || !chatInput) return;

  const dbName = @json($dbLabel);

  const append = (role, text) => {
    const wrap = document.createElement('div');
    wrap.className = 'h-ai-msg ' + role;
    const bubble = document.createElement('div');
    bubble.className = 'h-ai-bubble';
  ...
  const reply = (input) => {
    ...
    if (q.includes('rbac') || q.includes('role') || q.includes('permission')) {
      return 'Use Access & RBAC page in sidebar. Set module levels with radio controls, then save matrix.';
    }

    if (q.includes('user') || q.includes('import') || q.includes('export')) {
      return 'Use Users page in sidebar for create/update/delete and Excel import/export.';
    }

    if (q.includes('db') || q.includes('database')) {
      return 'Current active database is ' + dbName + '. You can inspect tables from Diagnostics > Database Browser.';
    }

    if (q.includes('notification') || q.includes('broadcast')) {
      return 'Use Notifications tab to send broadcast. New unread alerts also trigger browser sound and vibration where supported.';
    }

    if (q.includes('theme') || q.includes('logo') || q.includes('favicon') || q.includes('icon')) {
      return 'Use App Settings tab. Set theme color, logo, favicon, and app icon. Media Library helps quick selection.';
    }

    return 'Try asking about users, RBAC, notifications, diagnostics, or app branding. I can map you to the right section quickly.';
  };
  ...
  chatForm.addEventListener('submit', function (event) {
    ...
    append('user', text);
    chatInput.value = '';
    setTimeout(() => append('bot', reply(text)), 180);
  });
})();
</script>
@endsection
